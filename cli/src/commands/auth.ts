...
export function authCommands(program: Command) {
  ...
  auth
    .command("login")
    .description("Authenticate with ClawSkillHub")
  ...
  auth
    .command("logout")
    .description("Sign out of ClawSkillHub")
    .action(() => {
      clearToken();
  ...
  auth
    ...
    .action(async () => {
      ...
      if (!token) {
        ...
        console.log(chalk.gray("Run `csh auth login` to authenticate"));
        return;
      }
      ...
      if (error) {
        ...
        console.log(chalk.gray("Run `csh auth login` to re-authenticate"));
        return;
      }
